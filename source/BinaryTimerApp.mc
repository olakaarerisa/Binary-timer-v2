...
class BinaryTimerApp extends Application.AppBase {
    private var _timerView as BinaryTimerView?;
    ...
    public function getInitialView() as Array<Views or InputDelegates>? {
        _timerView = new $.BinaryTimerView(_backgroundData);
        var timerView = _timerView;
        if (timerView != null) {
            timerView.deleteBackgroundEvent();
            return [timerView, new $.BinaryTimerDelegate(timerView)] as Array<Views or InputDelegates>?;
        }
        return null;
    }
    ...
    public function getServiceDelegate() as Array<ServiceDelegate> {
        return [new $.BinaryTimerServiceDelegate()] as Array<ServiceDelegate>;
    }
...
function handleStorageUpdate() as Void {
    WatchUi.pushView(new $.BinaryTimerStorageChangedAlertView(), null, WatchUi.SLIDE_IMMEDIATE);
}